...
<header>
    @section('header')
        <a href="{{route('indexapplicant')}}" style="text-decoration: none">
            <div class="logo" >
                <span class="logo-black">Good</span><span class="logo-red">Job</span>
            </div>
        </a>
        <form class="search" method="get" action="{{route('search')}}">
            <input type="text" placeholder="Найдите вакансию вам по душе" name="search">
            <button type="submit">Найти</button>
        </form>
        @if(Auth::user())
            <div class="user-info">
                <span>{{Auth::user()->name}}</span>
                <a href="{{route('addsum')}}" style="text-decoration:none;">Добавить резюме</a>
                <a href="{{route('profile-page')}}" class="user-profile" style="text-decoration: none">Мой профиль</a>
                <a href="{{route('logout')}}" class="user-logout" style="text-decoration: none">Выход</a>
            </div>
        @else
        <div class="buttons">
